...
val test_sqrt = math.sqrt(2);System.out.println("""test_sqrt  : Double = """ + $show(test_sqrt ));$skip(19); 
println(test_sqrt);$skip(35); 
val numberList2 = (0 to 20).toList;System.out.println("""numberList2  : List[Int] = """ + $show(numberList2 ));$skip(91); 
for (num <- numberList2) {
...
val perfect_sq = numberList2.filter( (x: Int) => math.sqrt(x) % 1 == 0);System.out.println("""perfect_sq  : List[Int] = """ + $show(perfect_sq ));$skip(20); 
println(perfect_sq)}

object LearningScala3 {;import org.scalaide.worksheet.runtime.library.WorksheetSupport._; def main(args: Array[String])=$execute{;$skip(225); 
  ...
  def toupper(x: String) : String = {
  	x.toUpperCase()
  };System.out.println("""toupper: (x: String)String""");$skip(30); 
  val name = toupper("yifan");System.out.println("""name  : String = """ + $show(name ));$skip(16); 
  println(name);$skip(82); 
  
  def transformString(x: String, f: String => String) : String = {
  	f(x)
  };System.out.println("""transformString: (x: String, f: String => String)String""");$skip(49); val res$3 = 
  transformString("yifan", x => x.toUpperCase());System.out.println("""res3: String = """ + $show(res$3))}
}
